...
class OpenSmileFeatures(nn.Module):
    """OpenSmile Feature Extractor.
        This class converts a Log Mel Spectrogram to STFT then to audio file to extract 
        openSMILE features from the generated audio.
    Args:
        cfg: configuration file with the FFT parameters.
        device: cpu/cuda
    Returns:
        openSMILE features of shape (1, 6373)
    """

    def __init__(self, cfg, device):
        super().__init__()
        self.sr = cfg.sample_rate
        self.to_stft = torchaudio.transforms.InverseMelScale(
        ...
        self.to_audio = torchaudio.transforms.GriffinLim(
            n_fft=cfg.n_fft,
    ...
    def forward(self, x):
        x = self.to_stft(x.squeeze(0).exp())
        x = self.to_audio(x.to('cuda'))
        os_features = self.smile.process_signal(x.cpu().detach().numpy(), self.sr).to_numpy()
        os_features = torch.from_numpy(os_features)
        # if np.isnan(os_features.numpy()).any():
        #     os_features = torch.from_numpy(np.nan_to_num(os_features.numpy()))
        #     print(f"This Sample has nan in the embeddings, Total Samples")
        return os_features
...
class GaussianNoise(nn.Module):
    ...
    def __init__(self, ratio=0.3):
        self.ratio = ratio
        self.lambd = self.ratio * np.random.rand()

    def forward(self, x):
        # z = torch.normal(0, self.lambd, x.shape)
        # mixed = (1 - self.lambd) * x + z #+ torch.finfo(x.dtype).eps
        print('Pass')
        return x
